...
def demo_checkill():
    ...
    for path in img_path:
        # print path
        img = cv2.imread(path)
        img = cv2.imread(r"./image/test-14.jpg")
        h,w = img.shape[:2]
        ...
        for shape in shapes:
            if True:
                ...
                print mean_b,mean_g,mean_r,np.mean([mean_r,mean_g,mean_b])
...
def checkPKl():
    ...
    for i,value in enumerate(container):
        ...
        Result = value[imgPath]

        detectRes ,rgb= Result
        shapes,rect = detectRes
        if i % 100 == 0:
            print "lightcount = ", i
        for shape in shapes:
            # print rgb
            ...
            if avr_light < 60:
                ...
                newPath = os.path.join(save_dir, "light")
            # elif (max_light > 180 and min_light < 30):
            ...
            info = "%.f_%.f_%.f_%d.jpg" % (mean_b, mean_g, mean_r, i)
            cv2.imwrite(os.path.join(newPath,info),img)
            # cv2.imshow("img", img)
            # cv2.waitKey(0)
